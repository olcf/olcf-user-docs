...
TAU is a portable profiling and tracing toolkit for the performance
analysis of parallel programs written in C, C++, Fortran and other
languages. The instrumentation of programs can be done by binary code
rewriting, manual compiler directives, or automatic source code
transformation. TAU can support many parallel programming interfaces,
including MPI, OpenMP, pthreads, and ROCm.  TAU includes `paraprof`
which is a profile visualization tool, and generated traces can be
displayed by the Vampir, Paraver or JumpShot (included) visualization
tools.

https://www.cs.uoregon.edu/research/tau/home.php
https://www.cs.uoregon.edu/research/tau/docs.phpx
...
The TAU environment variables show the location of the TAU
installation and the setting for the default TAU_MAKEFILE.  TAU
compiler wrappers use the TAU_MAKEFILE for capabilities supported
...
Simple run using tau_exec
-------------------------

The simplest way to profile with TAU is to use the ``tau_exec``
command with an executable.  There is no need to recompile code with
this approach.  The ``tau_exec`` preloads TAU's dynamic library object
that intercepts and adds instrumentation at to known libraries at
runtime.  This method uses uses statistical sampling to estimate time
spent in known functions, such as MPI, OpenMP, or pthreads.  Note,
using ``tau_exec`` does not profile user-defined code, so its
...
The ``tau_exec`` tool will profile the code, tracking the time 
...
This example uses the included Jumpshot trace visualization tool.  TAU
can also create traces for Chrome/Pefetto (json) and for Vampir (otf2)
visualization.
...
srun -A <ACCOUNT> -N 1 -t 5 -n 2 tau_exec ./computePi-mpicxx
...
Automatic source instrumentation using `tau_compiler.sh`
--------------------------------------------------------

This approach uses TAU shell scripts (`tau_cc.sh`, `tau_cxx.sh`,
`tau_f90.sh`) to recompile code, automatically adding timer
start/stop calls into a copy of the code.  The Program Database
Toolkit (PDT) is used to parse the source code and add
instrumentation.  A selective filter file can be used to reduce
overhead and specify areas for instrumentation.
...
Profiling and tracing this execution will follow the earlier example.


Manual source instrumentation
------------------------------

TAU provides a rich set of functions that can be used to instrument code at very specific locations.   
The TAU documentation gives a list of all the functions available to instrument your code.
...
Some previous TAU training on the OLCF Training archive https://docs.olcf.ornl.gov/training/training_archive.html
...
In this section, some approaches for profiling and tracing will be presented.  
